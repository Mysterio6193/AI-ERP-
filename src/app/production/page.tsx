...
export default function ProductionPage() {
  ...
  return (
    <AppShell title="Production" breadcrumbs={[{ label: "Production" }]}>
      <div className="space-y-6">
        <PageHeader
          title="Manufacturing & Work Orders"
          description="Bills of Materials (BOM), batch production runs, capacity, and lot traceability."
          actions={
            <>
              <Button
                variant="outline"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard
            ...
            value={`${metrics.inProgressRuns} running`}
            description={`${metrics.totalRuns} total scheduled`}
            icon={Factory}
          />
          <KpiCard
            title="Overall Yield Rate"
            value={`${metrics.overallYield}%`}
            description={`${metrics.completedRuns} batches completed`}
            icon={TrendingUp}
          />
          <KpiCard
            title="Active Recipes (BOMs)"
            value={`${metrics.totalRecipes} BOMs`}
            description="Multi-stage formulations"
            icon={ChefHat}
          />
          <KpiCard
            title="Total Material Cost"
            value={money(metrics.totalMaterialCost)}
            description="Automated COGS deduction"
            icon={Layers}
          />
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-3 max-w-md">
            <TabsTrigger value="runs">Work Orders</TabsTrigger>
            <TabsTrigger value="recipes">BOM Recipes</TabsTrigger>
          ...
          <TabsContent value="runs" className="space-y-3">
            {!runs.length ? (
              <EmptyState
                ...
                description="Select a recipe from the Recipes tab or click Plan Run above to schedule manufacturing."
                action={
                  <Button
                    size="sm"
                    variant="outline"
              ...
              runs.map((run) => (
                <Card key={run.id} className="border-border shadow-sm transition-all hover:shadow-md">
                  <CardContent className="space-y-3 pt-5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-mono font-semibold text-xs text-foreground">{run.orderNumber}</span>
                          <Badge variant={STATUS_TONE[run.status] || "outline"} className="text-[10px] uppercase font-semibold">
                        ...
                        <p className="mt-1.5 text-sm font-semibold text-foreground">{run.product.name}</p>
                    ...
                    {expanded === run.id ? (
                      <div className="space-y-2 rounded-lg border border-border bg-muted/40 p-3">
                        <p className="text-xs font-semibold text-foreground">Required Ingredients & Bill of Materials</p>
                        <div className="space-y-1.5">
                          {run.consumptions.map((line) => (
                            <div
                              key={line.id}
                              className="flex flex-wrap items-center justify-between gap-2 text-xs bg-card rounded-md p-2 border border-border"
                            >
                              <span className="font-medium text-foreground">{line.component.name}</span>
                              <span className="text-muted-foreground">
                                Planned: <strong className="text-foreground">{line.plannedQty}</strong>
                        ...
                        {run.status === "completed" ? (
                          <div className="flex justify-between items-center border-t border-border pt-2 text-xs font-semibold text-foreground">
          ...
          <TabsContent value="recipes" className="space-y-3">
            {!recipes.length ? (
              <EmptyState
                ...
                description="Create your first Bill of Materials (BOM) to start planning manufacturing runs."
                action={
                  <Button
                    size="sm"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white"
              ...
              recipes.map((recipe) => (
                <Card key={recipe.id} className="border-border shadow-sm transition-all hover:shadow-md">
                  ...
                  <CardContent className="space-y-3">
                    {recipe.capacity?.limitedBy && recipe.capacity.batches < 3 ? (
                      <div className="flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-2.5 text-xs text-amber-900 dark:text-amber-200">
                    ...
                    <div className="space-y-1.5 rounded-lg border border-border bg-muted/40 p-3">
                      <p className="text-xs font-semibold text-foreground mb-1">Formulation & Bill of Materials</p>
                    ...
                    {recipe.instructions ? (
                      <div className="rounded-lg border border-dashed border-border p-3 text-xs text-muted-foreground">
                        <span className="font-semibold text-foreground">Standard Operating Procedure: </span>
                        {recipe.instructions}
                      </div>
                    ) : null}
                  </CardContent>
                  ...
                  <div className="space-y-4 pt-2">
                      ...
                      <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4 space-y-2">
                        ...
                        <div className="space-y-1.5">
                          {trace.shippedTo.slice(0, 20).map((row, index) => (
                            <div key={index} className="flex justify-between items-center gap-2 text-xs bg-card p-2 rounded-md border border-border">
                              <span className="font-medium text-foreground">
                                {row.customer}
                                {row.phone ? ` (${row.phone})` : ""}
                              </span>
                              <span className="text-muted-foreground">
                                Order <strong className="text-foreground">{row.orderNumber}</strong> · {row.quantity} × {row.product}
                              </span>
                            </div>
                          ))}
                        </div>
                    ...
                    {trace.usedIn.length ? (
                      <div className="rounded-lg border border-border bg-muted/40 p-4 space-y-2">
                        <p className="text-xs font-semibold text-foreground uppercase tracking-wider">Used in Downstream Production Runs</p>
                        <div className="space-y-1">
                          {trace.usedIn.map((row, index) => (
                            <p key={index} className="text-xs text-foreground bg-card p-2 rounded-md border border-border">
                              Run <strong className="text-foreground">{row.orderNumber}</strong>: {row.qty} of {row.component} used to formulate {row.product}
                            </p>
                          ))}
                        </div>
                      </div>
                    ...
                    {trace.producedAs.map((run) => (
                      <div key={run.orderNumber} className="rounded-lg border border-border bg-card p-4 space-y-2">
                        <p className="text-xs font-semibold text-foreground">
                          Work Order <strong className="text-foreground">{run.orderNumber}</strong> produced {run.producedQty} × {run.product}
                        </p>
                        <div className="space-y-1">
                          {run.madeFrom.map((line, index) => (
                            <div key={index} className="flex justify-between gap-2 text-xs text-muted-foreground bg-muted/50 p-1.5 rounded-md">
                    ...
                    {!trace.usedIn.length && !trace.producedAs.length ? (
                      <p className="rounded-lg border border-dashed border-border p-4 text-xs text-muted-foreground text-center">
            ...
            <div className="space-y-4 py-2">
              <div className="rounded-lg border border-border bg-muted/40 p-3 text-xs space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-foreground">{selectedRunForComplete.plannedQty} units</span>
                </div>
              </div>
            ...
            <div className="space-y-2 border-t border-border pt-3">
              ...
              <div className="space-y-2">
                {bomLines.map((line, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-muted/40 p-2.5 rounded-lg border border-border">
                    <div className="flex-1">
                      <Select
                        value={line.componentId}
                        onValueChange={(val) => updateBomLine(idx, "componentId", val)}
                      >
                        <SelectTrigger className="bg-card text-xs"><SelectValue placeholder="Select Component Product" /></SelectTrigger>
                        <SelectContent>
                          {products.map((p) => (
                            <SelectItem key={p.id} value={p.id}>
                              {p.name} ({p.sku})
